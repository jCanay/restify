...
import { BadgeCheck, BadgeMinus, BadgeQuestionMark, BadgeX, Pencil } from "lucide-react";
...
export default function BookingCMSearch() {
	...
	const colDefs = useMemo(() => [
		{
			...
			getQuickFilterText: params => {
				const config = STATUS_CONFIG[params.value] || STATUS_CONFIG.unknown;
	...
	const parseBookings = useMemo(() => (bookings) => {
		return bookings.map((e) => {
			...
			const displayName = e.account?.name
				...
				: isCurrentUser ? `${e.user.username} (Tú)` : e.user.username;
	...
	const onGridReady = (params) => {
		const dataSource = {
			...
			getRows: async (getRowsParams) => {
				...
				const response = await getAllBookingsByRestaurantId(getUserDefaultRestaurant()?.id, {
					page: page,
					size: size,
					sort: sortString,
					search: searchText
