...
function ScheduleForm({ modifiable = false, expanded = false }) {
    ...
    return (
        <div className={`schedule-form ${expanded && "expanded"}`}>
                    ...
                    <DropdownMenu>
                        <TooltipTrigger asChild>
                            <DropdownMenuTrigger asChild>
                                <button
                                    className={`${day.isClosed && "closed"}`}
                                >
                                    {days[day.dayOfWeek - 1]}{" "}
                                    {!day.isClosed || modifiable ? (
                                        <ChevronDown size={16} />
                                    ) : (
                                        ""
                                    )}
                        ...
                        {modifiable || !day.isClosed ? (
                            <DropdownMenuContent
                                className={`schedule-form-content`}
                            >
                                {!day.isClosed ? (
                                    <>
                                        <div
                                            className={`time ${expanded && "expanded"}`}
                                        >
                                            {day.slots.map(
                                                (slot, slotIndex) => (
                                                    <div
                                    ...
                                    <div className="italic text-destructive">
                                        Cerrado
                                    </div>
                                )}

                                {modifiable && day.slots.length < 8 && (
                                    <button
                                        className="add"
                                        onClick={() => handleAddClick(dayIndex)}
                                    >
                                        Añadir turno <ClockPlus size={14} />
                                    </button>
                                )}
                            </DropdownMenuContent>
                        ) : (
