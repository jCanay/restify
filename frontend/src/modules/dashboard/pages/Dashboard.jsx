...
function Dashboard() {
	...
	useEffect(() => {
		...
		const updateOffsetsManual2 = () => {
			const toasts = notificationsRef.current.querySelectorAll('li[data-sonner-toast]');
				...
				<DropdownMenu>
					<DropdownMenuTrigger asChild>
						<button className="user-btn">
							<img src={userPfp} alt="" />
							<div className="info">
								<h4>{account?.name || account?.surname ? `${account?.name || ""} ${account?.surname || ""}` : `${user?.username}`}</h4>
								<small>{roles[user?.role?.name]}</small>
							</div>
						</button>
					</DropdownMenuTrigger>
					<DropdownMenuContent className="user-content" side="right" align="end" sideOffset={16}>
						...
						<ul>
							<AlertDialog>
								<AlertDialogTrigger asChild>
									<DropdownMenuItem onSelect={(e) => e.preventDefault()} className="menu-item logout">
										<LogOut size={18} />
										<span>Cerrar sesión</span>
									</DropdownMenuItem>
								</AlertDialogTrigger>
								<AlertDialogContent className="logout-dialog" size="sm">
									<AlertDialogHeader>
									...
									<AlertDialogFooter>
										...
										<AlertDialogAction onClick={handleLogout}>Sí</AlertDialogAction>
									</AlertDialogFooter>
								</AlertDialogContent>
							</AlertDialog>
						</ul>
				...
				<div className="topbar">
					...
					<select name="" id="">
						{restaurants.map((restaurant, index) => (
							<option key={index} value={restaurant.id}>{restaurant.name}</option>
						))}
					</select>
				</div>
				<Outlet />
