...
function RestaurantAddressForm({ onBackClick, isValid, shakeTrigger }) {
	...
	const handleBlur = (target) => {
		setSetupDataAddress({ ...addresses[0], [target.id]: target.value.trim() });
	};

	useEffect(() => {
		const validate = () => {
			if (shakeTrigger > 0) {
				setStreetAddressValid(addresses[0].streetAddress.trim().length > 0);
		...
		<div className="restaurant-direction-form">
			...
			<form id="setup-form">
				<div>
					...
					/>
					{!isValid && !streetAddressValid && <p className="error">Este campo es obligatorio.</p>}
				</div>
				<div>
